...
def generate_reference_image(reference_file, output_path, blur_radius_microns, config):
    """
    1) Read the single tile from reference_file (ND2).
    2) Apply East-shadows filter + Gaussian blur + normalization.
    3) Replicate the resulting reference tile in an nrows x ncols mosaic with 
       overlap_percent from config["reference_mosaic"].
    4) Save final mosaic as reference.tif (optional).
    """
    with ND2Reader(reference_file) as ref_nd2:
        # Grab the single tile (assume v=0, c=2)
        raw_ref = np.nan_to_num(ref_nd2.get_frame_2D(v=0, c=2))
        pixel_size_microns = ref_nd2.metadata['pixel_microns']

    # 1) East-shadows filter
    filtered_ref = apply_east_shadows_filter(raw_ref)

    # 2) Gaussian blur
    sigma_pixels = blur_radius_microns / pixel_size_microns
    from skimage.filters import gaussian
    blurred_ref = gaussian(filtered_ref, sigma=sigma_pixels, preserve_range=True)

    # 3) Normalize (same logic you had before)
    blurred_ref_max = blurred_ref.max()
    raw_max = filtered_ref.max()
    if blurred_ref_max <= 0:
        raise ValueError("Blurred reference has no valid data.")
    # Scale
    blurred_ref_scaled = blurred_ref * (raw_max / blurred_ref_max)
    max_val = blurred_ref_scaled.max()
    if max_val <= 0:
        raise ValueError("Reference image has no valid intensity data after preprocessing.")
    normalized_tile = blurred_ref_scaled / max_val  # 0..1

    # 4) Replicate in mosaic form, using the same geometry as your real data
    nrows = config["reference_mosaic"]["nrows"]
    ncols = config["reference_mosaic"]["ncols"]
    overlap_percent = config["reference_mosaic"]["overlap_percent"]

    ref_mosaic = assemble_reference_mosaic(
        normalized_tile,
        nrows=nrows,
        ncols=ncols,
    ...
    # 5) Save final mosaic to disk (optional)
    imwrite(output_path, ref_mosaic.astype(np.float32))
    print(f"Reference mosaic saved to: {output_path}")

    return ref_mosaic


def assemble_reference_mosaic(
    ref_tile,
    nrows,
    ncols,
    overlap_percent=5.0
    ...
    """
    Create a mosaic of size nrows x ncols from a single reference tile, using
    a naive overlap approach. If each tile is (H, W), and overlap_percent=5.0,
    then horizontally we shift each new tile by (W - 0.05*W) = 0.95*W,
    and similarly for vertical.

    The final mosaic will have shape:
       final_height = H + (nrows - 1) * (H - overlap_y)
       final_width  = W + (ncols - 1) * (W - overlap_x)
    where overlap_x = overlap_percent/100 * W
          overlap_y = overlap_percent/100 * H
    
    ref_tile : 2D ndarray
        The single reference image/tile (shape H×W).
    nrows, ncols : int
        Number of times to repeat vertically/horizontally.
    overlap_percent : float
        Overlap in percent. E.g. 5.0 means 5% overlap.

    Returns
    -------
    ref_mosaic : 2D ndarray
        A “stitched” mosaic of shape roughly
        (H + (nrows - 1)*(H - overlap_y)) x
        (W + (ncols - 1)*(W - overlap_x)).
    """
    import numpy as np

    H, W = ref_tile.shape
    overlap_x = int(round(overlap_percent / 100.0 * W))
    overlap_y = int(round(overlap_percent / 100.0 * H))

    final_width = W + (ncols - 1) * (W - overlap_x)
    final_height = H + (nrows - 1) * (H - overlap_y)
    
    ref_mosaic = np.zeros((final_height, final_width), dtype=ref_tile.dtype)

    # Naively place the same ref_tile in each block
    for row in range(nrows):
        for col in range(ncols):
            start_y = row * (H - overlap_y)
            start_x = col * (W - overlap_x)
            ref_mosaic[start_y:start_y + H, start_x:start_x + W] = ref_tile

    return ref_mosaic


def ensure_same_shape(ref_mosaic, data_image):
    """
    If ref_mosaic.shape != data_image.shape, use skimage's resize 
    to match. We assume both are 2D arrays.
    
    Parameters
    ----------
    ref_mosaic : 2D ndarray
        The assembled reference mosaic (float or any numeric dtype).
    data_image : 2D ndarray
        The real data image we want to correct.

    Returns
    -------
    ref_matched : 2D ndarray
        The resized version of ref_mosaic, exactly the same shape 
        as data_image.
    """
    H_ref, W_ref = ref_mosaic.shape
    H_data, W_data = data_image.shape

    if (H_ref == H_data) and (W_ref == W_data):
        # Already the same shape, no resizing needed
        return ref_mosaic

    # Otherwise, do interpolation
    # preserve_range=True => keep the intensities rather than normalizing to 0..1
    # anti_aliasing can help if you are significantly shrinking the image
    print(f"Resizing reference from {ref_mosaic.shape} to {data_image.shape}")
    ref_resized = resize(
        ref_mosaic,
        (H_data, W_data),
        mode='reflect',
    ...
    with ND2Reader(fluorescence_path) as fluoro_nd2, ND2Reader(cars_path) as cars_nd2:
        ...
        def max_project_cars(
            ...
            for z_slice in range(total_z):
                ...
                ref_matched = ensure_same_shape(reference_image, correlated_sl)
        
                # 2) Divide by reference
                slice_div = np.nan_to_num(correlated_sl / ref_matched)
