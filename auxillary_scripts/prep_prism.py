...
FEATURES = [
    ("pure_lipid_count",        "Lipid"),
    ("lipofuscin_count",        "Lipofuscin"),
    ("lipid_lipofuscin_count",  "Lipidated Lipofuscin"),
]
...
def add_prism_sheets(infile: pathlib.Path, outfile: pathlib.Path) -> None:
    ...
    # ── NEW: Feature presence fractions (has vs none) ────────────────────────
    df_frac_long = build_feature_fraction_long(infile)

    # Long (per-donor) sheet
    name_long = "Fractions (long, per donor)"
    if name_long in wb.sheetnames:
        del wb[name_long]
    ws_long = wb.create_sheet(name_long)
    _write_df_simple(ws_long, df_frac_long)

    # Long (averages across donors) sheet
    name_long_avg = "Fractions (long, averages)"
    df_frac_avg = (
        df_frac_long
        .groupby(["condition", "cell_type", "feature"], as_index=False)[["has_pct", "none_pct"]]
        .mean()
    )
    if name_long_avg in wb.sheetnames:
        del wb[name_long_avg]
    ws_long_avg = wb.create_sheet(name_long_avg)
    _write_df_simple(ws_long_avg, df_frac_avg)

    # Wide per-donor sheets for Prism (Has / None)
    df_has_wide  = build_fraction_wide_dataframe(infile, state="has")
    df_none_wide = build_fraction_wide_dataframe(infile, state="none")

    specs = [
        ("Fractions (Has) — per donor",  df_has_wide,  "Has"),
        ("Fractions (None) — per donor", df_none_wide, "None"),
    ]
    for sheet_name, df_wide, state_title in specs:
        if sheet_name in wb.sheetnames:
            del wb[sheet_name]
        ws = wb.create_sheet(sheet_name)
        _write_fraction_headers(ws, state_title=state_title, start_col=2)
        _write_df(ws, df_wide, id_col=True)

    wb.save(outfile)
    
    
def build_feature_fraction_long(xlsx: pathlib.Path) -> pd.DataFrame:
    """
...
def build_fraction_wide_dataframe(xlsx: pathlib.Path, *, state: str = "has") -> pd.DataFrame:
    """
    Wide format per-donor table arranged for Prism:
      columns in blocks of len(CONDITIONS) per (CellType × Feature),
      values are has_pct or none_pct.
    """
    assert state in {"has", "none"}
    value_col = "has_pct" if state == "has" else "none_pct"
    long_df = build_feature_fraction_long(xlsx)

    donors = sorted(long_df["cond_donor"].dropna().unique().tolist())
    series_list = []

    # Column order: by CELL_TYPES, then FEATURES, then CONDITIONS
    for cell in CELL_TYPES:
        for _count_col, feat_label in FEATURES:
            for cond in CONDITIONS:
                mask = (
                    (long_df["cell_type"] == cell) &
                    (long_df["feature"] == feat_label) &
                    (long_df["condition"] == cond)
                )
                s = long_df.loc[mask, ["cond_donor", value_col]].set_index("cond_donor")[value_col]
                s = s.reindex(donors)
                series_list.append(s)

    wide = pd.concat(series_list, axis=1)
    wide.insert(0, "cond_donor", donors)
    return wide


def _write_fraction_headers(ws, state_title: str, start_col: int = 2):
    """
    Row 1 merges by (CellType × Feature), row 2 lists CONDITIONS under each group.
    """
    col = start_col
    for cell in CELL_TYPES:
        for _count_col, feat_label in FEATURES:
            ...
            ws.cell(row=1, column=start).value = f"{cell} — {feat_label} ({state_title})"
            for cond in CONDITIONS:
                ws.cell(row=2, column=col, value=cond)
                col += 1


def _write_df_simple(ws, df: pd.DataFrame):
    """Write a DataFrame with a single header row (no merges)."""
